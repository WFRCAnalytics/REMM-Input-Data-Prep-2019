...
class building:
    ...
    def convert_2_list(self):
        return [self.parcel_id, self.tazid, self.cid, self.jobs1, self.jobs2, self.jobs3, self.jobs4, self.jobs5, self.jobs6, self.jobs7, self.jobs8, self.jobs9, self.jobs10, self.jobs11]

    def export_2_csv(self):
        pass

def distribute_jobs(sector, buildings, njobs, btype='non_ind'):
    ...
    taz_jobs.loc[(taz_jobs['TAZID'] == tazid), 'jobs{}_distributed'.format(sector)] = jobs_distributed
    taz_jobs.loc[(taz_jobs['TAZID'] == tazid), 'jobs{}_leftover'.format(sector)] = remaining_jobs       
...
result = pd.DataFrame.spatial.from_featureclass(r'.\Outputs\job_spaces_2019_v2.gdb\_01_parcels_jobs_2019')
result = result[(result['building_type_id'].isin([3,4,5,6,7,9,10,11,13,15,16]) == True) & ((result['ind_spaces']>0) | (result['non_ind_spaces']>0))]
...
#taz_jobs = taz_jobs[taz_jobs['TAZID'] == 1577]
...
for t in taz_tuples:
    ...
    taz_parcels = result[(result['TAZID_900']== tazid)].copy()

    # create list of buildings
    global buildings
    buildings = [building(r.parcel_id_REMM, r.TAZID_900, r.CO_NAME, r.building_type, r.ind_spaces, r.non_ind_spaces) for r in taz_parcels.itertuples()]
    ...
    if len(buildings) > 0:
        ...
        distribute_jobs(1, buildings, jobs1, "non_ind")
        distribute_jobs(2, buildings, jobs2, "non_ind")
        distribute_jobs(3, buildings, jobs3, "non_ind")
        distribute_jobs(4, buildings, jobs4, "non_ind")
        distribute_jobs(5, buildings, jobs5, "ind")
        distribute_jobs(6, buildings, jobs6, "non_ind")
        distribute_jobs(7, buildings, jobs7, "non_ind")
        distribute_jobs(8, buildings, jobs8, "non_ind")
        distribute_jobs(9, buildings, jobs9, "non_ind")
        distribute_jobs(10, buildings, jobs10, "ind")
        distribute_jobs(11, buildings, jobs11, "non_ind")
...
df = pd.DataFrame(processed_buildings, columns =['building_id', 'taz_id', 'cid', 'jobs1', 'jobs2', 'jobs3', 'jobs4', 'jobs5', 'jobs6', 'jobs7', 'jobs8', 'jobs9', 'jobs10', 'jobs11'], dtype=int)
